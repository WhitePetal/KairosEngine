...
use kairos_ecs_macros::{Component, Event, Resource};

use crate::{
    change_detection::{DetectChanges, DetectChangesMut, Res, ResMut},
    error::Result,
    schedule::ApplyDeferred,
    system::{
        Commands, In, InMut, InRef, IntoSystem, Local, Query, RegisteredSystemError, SystemHandle,
        SystemHandleTemplate, SystemId, despawn_unused_registered_systems, system_value,
    },
    world::World,
};
...
fn change_detection() {
    ...
    fn count_up_iff_changed(mut counter: ResMut<Counter>, change_detector: ResMut<ChangeDetector>) {
...
fn nested_systems_with_inputs() {
    ...
    let increment_by = world.register_system(|In(amt): In<u8>, mut counter: ResMut<Counter>| {
        counter.0 += amt;
    });
    let nested_id = world.register_system(nested);
...
fn run_system_recursive() {
    std::thread_local! {
        ...
        static SYSTEM_ID: Cell<Option<SystemId>> = Default::default();
    }
