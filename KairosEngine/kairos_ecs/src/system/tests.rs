...
use crate::{
    ...
    bundle::Bundles,
    change_detection::{DetectChanges, NonSend, NonSendMut, Res, ResMut},
...
#[test]
fn query_set_system() {
    fn sys(mut _set: ParamSet<(Query<&'static mut A>, Query<&'static A>)>) {}
    let mut world = World::default();
    run_system(&mut world, sys);
}
...
fn conflicting_query_with_query_set_system() {
    fn sys(_query: Query<&mut A>, _set: ParamSet<(Query<&'static mut A>, Query<&'static B>)>) {}
...
fn conflicting_query_sets_system() {
    fn sys(
        _set_1: ParamSet<(Query<&'static mut A>,)>,
        _set_2: ParamSet<(Query<&'static mut A>, Query<&'static B>)>,
    ) {
    }
...
fn pipe_change_detection() {
    ...
    sys.run(Default::default(), &mut world).unwrap();
    ...
    let info1 = sys
        ...
        .unwrap();
    let info2 = sys.run(Default::default(), &mut world).unwrap();
...
#[should_panic(
    expected = "Encountered an error in system `kairos_ecs::system::tests::simple_fallible_system::sys`: error"
)]
...
#[cfg_attr(not(feature = "debug"), ignore)]
#[should_panic(
    expected = "Encountered an error in system `kairos_ecs::system::tests::simple_fallible_exclusive_system::sys`: error"
)]
