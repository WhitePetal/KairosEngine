...
use crate::{
    ...
    query::{
        ArchetypeFilter, ContiguousQueryData, IterQueryData, NopWorldQuery, QueryCombinationIter,
        QueryContiguousIter, QueryData, QueryEntityError, QueryFilter, QueryIter, QueryManyIter,
        QueryManyUniqueIter, QueryNotDenseError, QueryParIter, QueryParManyIter,
...
impl<'w, 's, D: QueryData, F: QueryFilter> Query<'w, 's, D, F> {
        ...
        D: IterQueryData,
    {
        ...
        match (first, extra) {
            (Some(r), false) => Ok(r),
            (None, _) => Err(QuerySingleError::NonEntities(DebugName::type_name::<Self>())),
            (Some(_), true) => Err(QuerySingleError::MultipleEntities(DebugName::type_name::<
                Self,
            >())),
        }
    }
