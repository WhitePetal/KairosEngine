...
fn observer_multiple_targets() {
    ...
    world.add_observer(|_: On<EntityComponentsEvent, (A, B)>, mut res: ResMut<R>| res.0 += 1000);
    // test all tuples
    world.add_observer(
...
fn observer_dynamic_trigger() {
    ...
    let observe = unsafe {
        Observer::with_dynamic_runner(|mut world, _observer, _trigger_context, _event, _trigger| {
...
fn observer_fully_dynamic_trigger() {
    ...
    let event_key = unsafe { EventKey::new(event_id) };

    // SAFETY: event_key was just created, observer reads event_data as u32
    let observe = unsafe {
        Observer::with_dynamic_runner(|mut world, _observer, _trigger_context, event, _trigger| {
...
fn observer_propagating() {
    ...
    world
        .entity_mut(parent)
        .observe(move |event: On<EventPropagating>, mut res: ResMut<Order>| {
            res.observed("parent");
