...
unsafe impl<T: Component> WorldQuery for With<T> {
    ...
    unsafe fn init_fetch<'w, 's>(
        _world: crate::world::unsafe_world_cell::UnsafeWorldCell<'w>,
        _state: &'s Self::State,
        _last_run: crate::change_detection::Tick,
        _this_run: crate::change_detection::Tick,
    ) -> Self::Fetch<'w> {
    }
    ...
    unsafe fn set_archetype<'w, 's>(
        ...
        _state: &'s Self::State,
        _archetype: &'w crate::archetype::Archetype,
        _table: &'w crate::storage::Table,
    ) {
    }
    ...
    ($(#[$meta:meta])* $(($filter: ident, $state: ident)),*) => {
        ...
        unsafe impl<$($filter: QueryFilter),*> WorldQuery for Or<($($filter,)*)> {
            ...
            fn update_component_access(state: &Self::State, access: &mut FilteredAccess) {
                ...
                new_access.required = std::mem::take(&mut access.required);
...
unsafe impl<T: Component> WorldQuery for Allow<T> {
    ...
    #[inline]
    fn update_component_access(id: &Self::State, access: &mut FilteredAccess) {
...
unsafe impl<T: Component> WorldQuery for Added<T> {
    ...
    unsafe fn set_table<'w, 's>(
        ...
        unsafe {
            fetch.ticks.set_table(table_ticks);
        }
    }
    ...
    unsafe fn filter_fetch(
        ...
        fetch.ticks.extract(
            |table| {
                ...
                unsafe { tick.deref() }.is_newer_than(fetch.last_run, fetch.this_run)
            },
            |sparse_set| {
