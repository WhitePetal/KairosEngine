...
use crate::{
    ...
    query::{Added, Allow, Changed, Has, Or, QueryState, With, Without},
    system::{Query, QueryLens, RunSystemOnce},
    world::{EntityMut, EntityRef, FilteredEntityMut, FilteredEntityRef, FromWorld, World},
};
...
fn dense_query_over_option_is_buggy() {
    ...
    let mut query = QueryState::<EntityRef>::new(&mut world).transmute::<Option<&Sparse>>(&world);
