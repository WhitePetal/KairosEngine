...
use crate::{
    ...
    world::{
        EntityMut, EntityMutExcept, EntityRefExcept, FilteredEntityMut, FilteredEntityRef, World,
    },
};
...
fn builder_provide_access() {
    ...
    let mut query =
        QueryBuilder::<(Entity, FilteredEntityRef, FilteredEntityMut), With<D>>::new(&mut world)
            .data::<&mut A>()
            .data::<&B>()
            .build();
    ...
    let mut query =
        QueryBuilder::<(Entity, FilteredEntityMut, FilteredEntityMut), With<D>>::new(&mut world)
            .data::<&mut A>()
            .data::<&B>()
            .build();
