...
/// The path to the `Cargo.toml` file for the Kairos project.
pub struct KairosManifest {
    manifest: Document<Box<str>>,
    modified_time: SystemTime,
}

const KAIROS: &str = "kairos";

impl KairosManifest {
    /// Calls `f` with a global shared instance of the [`KairosManifest`] struct.
    pub fn shared<R>(f: impl FnOnce(&KairosManifest) -> R) -> R {
        static MANIFESTS: RwLock<BTreeMap<PathBuf, KairosManifest>> = RwLock::new(BTreeMap::new());
    ...
    /// the [manifest](KairosManifest) by [name](str).
    pub fn maybe_get_path(&self, name: &str) -> Option<syn::Path> {
        ...
        let find_in_deps = |deps: &Item| -> Option<syn::Path> {
            ...
            } else if deps.get(KAIROS).is_some() {
                KAIROS
            } else {
            ...
            if let Some(module) = rust_name.strip_prefix("kairos_") {
