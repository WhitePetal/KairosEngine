...
pub fn derive_schedule_label(input: TokenStream) -> TokenStream {
    ...
    let mut trait_path = kairos_ecs_path();
...
/// Return the path to the Kairos ECS crate, relative to the caller's crate.
///
/// The ECS lives in its own `kairos_ecs` crate (flat bevy_ecs-style layout),
/// so this resolves that crate from the caller's manifest and returns it
/// as-is; no module segment is appended. Macro expansions can then append
/// module paths, e.g. `#kairos_ecs_path::world::World`. In-crate derive users
/// resolve through the `extern crate self as kairos_ecs;` alias at the crate
/// root (the manifest never lists the crate itself).
pub(crate) fn kairos_ecs_path() -> syn::Path {
    KairosManifest::shared(|manifest| manifest.get_path("kairos_ecs"))
}
