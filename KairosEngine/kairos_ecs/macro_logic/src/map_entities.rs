...
use syn::{
    Data, DataEnum, DataStruct, Expr, ExprPath, Ident, Member, Path, Result, Token, parse::Parse,
    spanned::Spanned,
};
...
pub fn map_entities(
    ...
    if let Some(map_entities_override) = map_entities_attr {
        let map_entities_tokens = map_entities_override.to_token_stream(kairos_ecs);
        return Some(quote! (
