...
macro_rules! encode_blocks {
    ...
    ($name:ident, U8, $block_w:expr, $block_h:expr, $block_size:expr, $block_fn:ident) => {
        pub fn $name(
            pixels: &$crate::texture::format::PixelDatas,
            width: usize,
            height: usize,
        ) -> $crate::texture::format::PixelDatas {
            ...
            let rgba: std::borrow::Cow<'_, [u8]> = match pixels {
                $crate::texture::format::PixelDatas::U8(data) => {
            ...
            out.par_chunks_mut($block_size).enumerate().for_each(
                |(i, chunk): (usize, &mut [u8])| {
                    let bx_i = i % bx;
                    let by_i = i / bx;
                    let block = $crate::texture::format::extract_block(
                        rgba.as_ref(),
                        width,
                        height,
                        bx_i * $block_w,
            ...
            $crate::texture::format::PixelDatas::U8(out)
        }
    };
    ...
    ($name:ident, F16, $block_w:expr, $block_h:expr, $block_size:expr, $block_fn:ident) => {
        pub fn $name(
            ...
            let rgba: std::borrow::Cow<'_, [half::f16]> = match pixels {
                $crate::texture::format::PixelDatas::F16(data) => {
                    std::borrow::Cow::Borrowed(data.as_slice())
                }
                $crate::texture::format::PixelDatas::U8(data) => {
                    const CHUNK: usize = 4096;
                ...
                $crate::texture::format::PixelDatas::U16(data) => {
                    const CHUNK: usize = 4096;
                    let n = data.len();
                    let mut out = vec![half::f16::ZERO; n];
                    out.par_chunks_mut(CHUNK)
                ...
                $crate::texture::format::PixelDatas::F32(data) => {
                    const CHUNK: usize = 4096;
                    let n = data.len();
                    let mut out = vec![half::f16::ZERO; n];
                    out.par_chunks_mut(CHUNK)
            ...
            out.par_chunks_mut($block_size).enumerate().for_each(
                |(i, chunk): (usize, &mut [u8])| {
                    let bx_i = i % bx;
                    let by_i = i / bx;
                    let block = $crate::texture::format::extract_block_f16(
                        rgba.as_ref(),
                        width,
                        height,
                        bx_i * $block_w,
            ...
            $crate::texture::format::PixelDatas::F16(out)
        }
    };
}
...
macro_rules! decode_blocks {
    ...
    ($name:ident, U8, $layout:expr, $decode_fn:ident) => {
        pub fn $name(
            data: &$crate::texture::format::PixelDatas,
            width: usize,
            height: usize,
        ) -> $crate::texture::format::PixelDatas {
            let raw = data.convert_to_u8_bytes();
            let $crate::texture::format::BlockLayout {
                w: block_w,
                h: block_h,
                bytes: block_size,
            } = $layout;
    ...
    ($name:ident, S8, $layout:expr, $decode_fn:ident) => {
        pub fn $name(
            ...
            $crate::texture::format::PixelDatas::S8(out)
        }
    };
    // U16 variant
