...
use crate::{ecs::{bundle::InsertMode, component::{ComponentCloneBehavior, ComponentDescriptor, StorageType}, entity::{ComponentCloneCtx, Entity, EntityCloner, SourceComponent}, hierarchy::{ChildOf, Children}, lifecycle::HookContext, world::{DeferredWorld, World}}, ptr::OwningPtr};
...
fn move_without_clone() {
    ...
    let mut world = World::default();
    let e = world.spawn((A, B(vec![1, 2, 3]))).id();
    let e_clone = world.spawn_empty().id();
    ...
    assert_eq!(world.get::<B>(e_clone), Some(&B(vec![1, 2, 3])));
}
...
fn move_with_remove_hook() {
    ...
    let mut world = World::default();
    let e = world.spawn(B(Some(vec![1, 2, 3]))).id();
    let e_clone = world.spawn_empty().id();
    let mut builder = EntityCloner::build_opt_out(&mut world);
    builder.move_components(true);
    let mut cloner = builder.finish();
...
fn move_relationship() {
    ...
    assert_eq!(
        ...
        Some(&Target(vec![e_source_moved]))
    );
...
fn clone_relationship_with_data() {
    ...
    let e_source = world
        ...
        .id();
    world.get_mut::<Target>(e_target).unwrap().data = vec![4, 5, 6];
    ...
    assert_eq!(target.target, vec![e_source]);
...
fn move_relationship_with_data() {
    ...
    let source_data = vec![1, 2, 3];
    let target_data = vec![4, 5, 6];
