...
pub struct AutoInsertApplyDeferredPass {
    ...
    auto_sync_node_ids: FixedHashMap<u32, SystemKey>,
}
...
impl ScheduleBuildPass for AutoInsertApplyDeferredPass {
    ...
    fn build(
        ...
        let mut system_has_conditions_cache = FixedHashMap::<SystemKey, bool>::default();
        ...
        let mut distances_and_pending_sync: FixedHashMap<SystemKey, (u32, bool)> =
            FixedHashMap::with_capacity_and_hasher(topo.len(), Default::default());

        // Keep track of any explicit sync nodes for a specific distance.
        let mut distance_to_explicit_sync_node: FixedHashMap<u32, SystemKey> =
