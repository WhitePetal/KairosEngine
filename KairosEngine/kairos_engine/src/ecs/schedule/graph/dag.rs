...
impl<N: GraphNodeId, S: BuildHasher> DagAnalysis<N, S> {
        ...
        S: Default,
    {
        ...
        let mut map = <FixedHashMap<_, _>>::with_capacity_and_hasher(n, Default::default());
        let mut topsorted =
        ...
        let mut connected = FixedHashSet::default();
    ...
    pub fn connected(&self) -> &FixedHashSet<(N, N), S> {
        &self.connected
    }
...
pub struct DagGroups<K, V, S = FixedHasher>(FixedHashMap<K, IndexSet<V, S>, S>);
...
impl<K: Eq + Hash, V: Clone + Eq + Hash, S: BuildHasher + Default> DagGroups<K, V, S> {
    ...
    where
        N: GraphNodeId + TryInto<K, Error = V>,
    {
        let mut groups: FixedHashMap<K, IndexSet<V, S>, S> =
            FixedHashMap::with_capacity_and_hasher(capacity, Default::default());
...
impl<K, V, S> Deref for DagGroups<K, V, S> {
    type Target = FixedHashMap<K, IndexSet<V, S>, S>;
