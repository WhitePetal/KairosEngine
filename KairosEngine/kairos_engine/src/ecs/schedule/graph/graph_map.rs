...
impl<const DIRECTED: bool, N: GraphNodeId, S: BuildHasher> Graph<DIRECTED, N, S> {
        ...
        S: Default,
    {
        Self {
            ...
            edges: FixedHashSet::with_capacity_and_hasher(edges, S::default()),
        }
    }
...
impl<N: GraphNodeId, S: BuildHasher> DiGraph<N, S> {
    ...
    pub fn simple_cycles_in_component(&self, scc: &[N]) -> Vec<Vec<N>> {
        ...
        while let Some(mut scc) = sccs.pop() {
            ...
            let mut blocked: FixedHashSet<_> =
                FixedHashSet::with_capacity_and_hasher(subgraph.node_count(), Default::default());
            ...
            let mut unblock_together: FixedHashMap<N, FixedHashSet<N>> =
                FixedHashMap::with_capacity_and_hasher(subgraph.node_count(), Default::default());
            ...
            let mut maybe_in_more_cycles: FixedHashSet<N> =
                FixedHashSet::with_capacity_and_hasher(subgraph.node_count(), Default::default());
            // stack for DFS
