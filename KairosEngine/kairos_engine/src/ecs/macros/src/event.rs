use kairos_macro_utils::fq_std::{FQInto, FQSend, FQSync};
...
pub fn derive_event(input: TokenStream) -> TokenStream {
    ...
    let kairos_ecs_path: Path = crate::kairos_ecs_path();
    ...
    let trigger = if let Some(trigger) = trigger {
        ...
        quote! {#kairos_ecs_path::event::GlobalTrigger}
    };
...
pub fn derive_entity_event(input: TokenStream) -> TokenStream {
    ...
    let trigger = if let Some(trigger) = trigger {
        ...
        let traversal = traversal
            .unwrap_or_else(|| parse_quote! {&'static #kairos_ecs_path::hierarchy::ChildOf});
        quote! {#kairos_ecs_path::event::PropagateEntityTrigger<#auto_propagate, Self, #traversal>}
    } else {
        quote! {#kairos_ecs_path::event::EntityTrigger}
    };

    let set_entity_event_target_impl = if propagate {
        quote! {
            impl #impl_generics #kairos_ecs_path::event::SetEntityEventTarget for #struct_name #type_generics #where_clause {
                fn set_event_target(&mut self, entity: #kairos_ecs_path::entity::Entity) {
    ...
    TokenStream::from(quote! {
        ...
        impl #impl_generics #kairos_ecs_path::event::EntityEvent for #struct_name #type_generics #where_clause {
            fn event_target(&self) -> #kairos_ecs_path::entity::Entity {
                #kairos_ecs_path::entity::ContainsEntity::entity(&self.#entity_field)
            }
        }
