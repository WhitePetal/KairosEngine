...
use crate::{
    ...
    ecs::{
        ...
        entity::Entity,
        query::{
            Added, AnyOf, ArchetypeFilter, ArchetypeQueryData, Changed, Has, Or,
            QueryCombinationIter, QueryState, ReadOnlyQueryData, With, Without,
        },
...
fn check_combinations<T: Ord + Hash + Debug, const K: usize>(
    values: FixedHashSet<[&T; K]>,
    expected: FixedHashSet<[&T; K]>,
) {
...
fn query_iter_combinations() {
    ...
    let values: FixedHashSet<[&A; 2]> = world.query::<&A>().iter_combinations(&world).collect();
    check_combinations(
        values,
        FixedHashSet::from([
            [&A(1), &A(2)],
    ...
    let values: FixedHashSet<[&A; 3]> = a_query.iter_combinations(&world).collect();
    check_combinations(
        values,
        FixedHashSet::from([
            [&A(1), &A(2), &A(3)],
...
fn query_filtered_iter_combinations() {
    ...
    let values: FixedHashSet<[&A; 2]> = a_wout_b.iter_combinations(&world).collect();
    check_combinations(
        values,
        FixedHashSet::from([[&A(2), &A(3)], [&A(2), &A(4)], [&A(3), &A(4)]]),
    );

    let values: FixedHashSet<[&A; 3]> = a_wout_b.iter_combinations(&world).collect();
    check_combinations(values, FixedHashSet::from([[&A(2), &A(3), &A(4)]]));

    let mut query = world.query_filtered::<&A, Or<(With<A>, With<B>)>>();
    let values: FixedHashSet<[&A; 2]> = query.iter_combinations(&world).collect();
    check_combinations(
        values,
        FixedHashSet::from([
            [&A(1), &A(2)],
    ...
    let values: FixedHashSet<[&A; 3]> = a_wout_b.iter_combinations(&world).collect();
    check_combinations(values, FixedHashSet::from([[&A(12), &A(103), &A(1004)]]));
...
fn query_iter_combinations_sparse() {
    ...
    let values: FixedHashSet<[&Sparse; 3]> =
...
fn has_query() {
    ...
    let values: FixedHashSet<(&A, bool)> = world.query::<(&A, Has<B>)>().iter(&world).collect();
