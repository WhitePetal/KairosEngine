...
pub struct SceneEntityReference(FixedHashed<InnerSceneEntityReference>);
...
impl SceneEntityReference {
    pub fn new(
        ...
        Self(FixedHashed::new(InnerSceneEntityReference {
            file,
            line,
            column,
            name_id,
...
impl std::fmt::Display for SceneEntityReference {
...
impl Deref for SceneEntityReference {
    type Target = FixedHashed<InnerSceneEntityReference>;
...
impl Equivalent<FixedHashed<InnerSceneEntityReference>> for SceneEntityReference {
    #[inline]
    fn equivalent(&self, key: &FixedHashed<InnerSceneEntityReference>) -> bool {
        &self.0 == key
    }
}
...
impl Template for EntityTemplate {
    ...
    fn build_template(&self, context: &mut TemplateContext) -> Result<Self::Output> {
        Ok(match self {
            EntityTemplate::Entity(entity) => *entity,
            EntityTemplate::SceneEntityReference(reference) => context.get_entity(*reference),
            EntityTemplate::None => {
                return Err(KairosError::error(
    ...
    fn clone_template(&self) -> Self {
        match self {
            EntityTemplate::Entity(entity) => Self::Entity(*entity),
            EntityTemplate::SceneEntityReference(reference) => {
                Self::SceneEntityReference(*reference)
            }
            EntityTemplate::None => Self::None,
        }
    }
}
