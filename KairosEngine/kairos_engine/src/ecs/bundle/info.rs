...
pub struct Bundles {
    ...
    dynamic_bundle_ids: FixedHashMap<Box<[ComponentId]>, BundleId>,
    dynamic_bundle_storages: FixedHashMap<BundleId, Vec<StorageType>>,
    /// Cache optimized dynamic [`BundleId`] with single component
    dynamic_component_bundle_ids: FixedHashMap<ComponentId, BundleId>,
    dynamic_component_storages: FixedHashMap<BundleId, StorageType>,
}
    ...
    pub(crate) unsafe fn register_contributed_bundle_info<T: Bundle>(
        ...
        if let Some(id) = self.contributed_bundle_ids.get(&TypeId::of::<T>()).cloned() {
            ...
            let id = unsafe {
                ...
                self.init_dynamic_info(storages, components, std::slice::from_raw_parts(ptr, len))
            };
