...
impl MessageRegistry {
    ...
    pub fn run_updates(&mut self, world: &mut World, last_change_tick: Tick) {
        for registered_message in &mut self.message_updates {
            ...
            {
                let has_changed = messages.has_changed_slice(last_change_tick);
                if registered_message.previously_update || has_changed {
                    ...
                    registered_message.previously_update =
                        has_changed || !registered_message.previously_update
                }
            }
        }
    }
