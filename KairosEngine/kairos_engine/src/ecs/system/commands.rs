...
const _: () = {
    ...
    pub struct FetchState {
        state: <__StructFieldsAlias<'static, 'static> as ecs::system::SystemParam>::State,
    }
    // SAFETY: Only reads Entities
    unsafe impl ecs::system::SystemParam for Commands<'_, '_> {
        ...
        fn init_state(world: &mut World) -> Self::State {
            FetchState {
                state: <__StructFieldsAlias<'_, '_> as ecs::system::SystemParam>::init_state(world),
            }
        }
        ...
        fn init_access(
            ...
            component_access_set: &mut ecs::query::FilteredAccessSet,
            world: &mut World,
        ) {
            <__StructFieldsAlias<'_, '_> as ecs::system::SystemParam>::init_access(
                &state.state,
                system_meta,
        ...
        fn apply(state: &mut Self::State, system_meta: &super::SystemMeta, world: &mut World) {
            <__StructFieldsAlias<'_, '_> as ecs::system::SystemParam>::apply(
                &mut state.state,
                system_meta,
                world,
            );
        ...
        fn queue(
            ...
            world: ecs::world::DeferredWorld,
        ) {
            <__StructFieldsAlias<'_, '_> as ecs::system::SystemParam>::queue(
                &mut state.state,
                system_meta,
                world,
            );
        ...
        unsafe fn get_param<'world, 'state>(
            ...
            system_meta: &super::SystemMeta,
            world: ecs::world::unsafe_world_cell::UnsafeWorldCell<'world>,
            change_tick: ecs::change_detection::Tick,
        ) -> Result<Self::Item<'world, 'state>, super::SystemParamValidationError> {
            let params = unsafe {
                <__StructFieldsAlias as ecs::system::SystemParam>::get_param(
                    &mut state.state,
                    system_meta,
                    world,
                    change_tick,
    ...
    // SAFETY: Only reads Entities
    unsafe impl<'w, 's> ecs::system::ReadOnlySystemParam for Commands<'w, 's>
    where
        Deferred<'s, CommandQueue>: ecs::system::ReadOnlySystemParam,
        &'w Entities: ecs::system::ReadOnlySystemParam,
    {
    }
};
...
impl<'a> EntityCommands<'a> {
    ...
    pub fn insert_if_neq<T: Component + PartialEq>(&mut self, component: T) -> &mut Self {
        ...
        self.queue(move |mut entity: EntityWorldMut| {
            if entity
                .get::<T>()
                .is_none_or(|old_componenet| *old_componenet != component)
            {
