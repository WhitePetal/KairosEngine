...
#[track_caller]
pub fn observe<M>(observe: impl IntoEntityObserver<M>) -> impl EntityCommand {
    let caller = MaybeLocation::caller();
    move |mut entity: EntityWorldMut| {
        entity.observe_with_caller(observe, caller);
    }
}
...
pub fn log_components() -> impl EntityCommand {
    move |entity: EntityWorldMut| {
        ...
        let mut components: Vec<_> = entity
            .world()
            .inspect_entity(id)
            .expect("Entity existence is verfied before an EntityCommand is executed")
        ...
        #[cfg(not(feature = "debug"))]
        {
            ...
            #[cfg(not(feature = "trace"))]
            {
                let name = name
                    ...
                    .unwrap_or_default();
                info!(
                    "Entity {id}{name}: {component_count} components. Enable the `debug` feature to log component names."
                );
            }
        }
