...
use crate::{
    ...
    ecs::{
        ...
        resource::Resource,
        system::{
            DynSystemParam, DynSystemParamState, FromInput, FunctionSystem, If, IntoResult,
            IntoSystem, Local, ParamSet, Query, ReadOnlySystem, System, SystemInput, SystemMeta,
            SystemParam, SystemParamFunction, SystemParamValidationError, SystemState,
        },
        world::{
    ...
    Builder: SystemParamBuilder<Func::Param> + Send + Sync + 'static,
{
    ...
    #[inline]
    fn name(&self) -> crate::debug::DebugName {
        match &self.inner {
    ...
    ) -> Result<Self::Out, super::RunSystemError> {
    ...
    #[inline]
    fn queue_deferred(&mut self, world: crate::ecs::world::DeferredWorld) {
        match &mut self.inner {
    ...
    #[inline]
    fn initialize(&mut self, world: &mut World) -> crate::ecs::query::FilteredAccessSet {
...
{
    fn build(self, world: &mut World) -> <Query<'w, 's, D, F> as SystemParam>::State {
...
{
    fn build(self, world: &mut World) -> <ParamSet<'w, 's, Vec<P>> as SystemParam>::State {
        self.0
            .into_iter()
            .map(|builder| builder.build(world))
            .collect()
