...
use kira::{AudioManager, AudioManagerSettings, Decibels, DefaultBackend, Easing, Mapping, Mix, Value, effect::{filter::FilterBuilder, reverb::ReverbBuilder}, modulator::tweener::{TweenerBuilder, TweenerHandle}, sound::static_sound::{StaticSoundData, StaticSoundSettings}, track::TrackBuilder};
...
impl AudioEngine {
    ...
    pub fn example1(&mut self) -> Result<TweenerHandle, Box<dyn std::error::Error>> {
        let underwater_tweener = self.manager.add_modulator(TweenerBuilder { initial_value: 0.0 })?;
        let mut lead_track = self.manager.add_sub_track(
            TrackBuilder::new()
        ...
        let common_sound_settings = StaticSoundSettings::new()
            .loop_region(music_duration / 2.0..music_duration);
        let arp = StaticSoundData::from_file("res/audios/arp.ogg")?
        ...
        let lead = StaticSoundData::from_file("res/audios/lead.ogg")?
            .with_settings(common_sound_settings);
