...
//!
//! Everything runs on a bare [`World`] — `schedule::install` + `graphics::install`,
//! one frame being `run_schedule(Main)` + `clear_trackers()`. `Engine::new()` is
//! deliberately avoided (it builds an audio device), and the rails never touch
//! wgpu, so the whole suite is headless.
//!
...
use crate::{
    ...
    kairos_editor::schedule::{self, Main},
...
/// Schedules first (they create the `Extract` stage), then the render rails
/// that register into it — the same order `Engine::new` uses.
fn boot() -> World {
...
fn projection_of(world: &World, camera: Entity) -> Option<crate::math::float4x4> {
    world
        .entity(camera)
        .get::<CameraView>()
...
fn expected_projection(world: &World, camera: Entity, aspect: f32) -> crate::math::float4x4 {
    let transform = *world
        .entity(camera)
        .get::<LocalTransform>()
