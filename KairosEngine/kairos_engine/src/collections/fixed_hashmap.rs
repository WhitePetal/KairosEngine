...
#[repr(transparent)]
pub struct FixedHashMap<K, V, S = FixedHasher>(hashbrown::HashMap<K, V, S>);

impl<K, V, S> Clone for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: Clone,
{
    #[inline]
    fn clone(&self) -> Self {
        Self(self.0.clone())
...
impl<K, V, S> Debug for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: Debug,
{
    #[inline]
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        <hashbrown::HashMap<K, V, S> as Debug>::fmt(&self.0, f)
    }
}

impl<K, V, S> Default for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: Default,
{
    #[inline]
    fn default() -> Self {
...
impl<K, V, S> PartialEq for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: PartialEq,
{
    #[inline]
...
impl<K, V, S> Eq for FixedHashMap<K, V, S> where hashbrown::HashMap<K, V, S>: Eq {}

impl<K, V, S, T> FromIterator<T> for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: FromIterator<T>,
{
    #[inline]
...
impl<K, V, S, T> Index<T> for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: Index<T>,
{
    type Output = <hashbrown::HashMap<K, V, S> as Index<T>>::Output;
...
impl<K, V, S> IntoIterator for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: IntoIterator,
{
    type Item = <hashbrown::HashMap<K, V, S> as IntoIterator>::Item;
    type IntoIter = <hashbrown::HashMap<K, V, S> as IntoIterator>::IntoIter;
...
impl<'a, K, V, S> IntoIterator for &'a FixedHashMap<K, V, S>
where
    &'a hashbrown::HashMap<K, V, S>: IntoIterator,
{
    type Item = <&'a hashbrown::HashMap<K, V, S> as IntoIterator>::Item;

    type IntoIter = <&'a hashbrown::HashMap<K, V, S> as IntoIterator>::IntoIter;
...
impl<'a, K, V, S> IntoIterator for &'a mut FixedHashMap<K, V, S>
where
    &'a mut hashbrown::HashMap<K, V, S>: IntoIterator,
{
    type Item = <&'a mut hashbrown::HashMap<K, V, S> as IntoIterator>::Item;

    type IntoIter = <&'a mut hashbrown::HashMap<K, V, S> as IntoIterator>::IntoIter;
...
impl<K, V, S, T> Extend<T> for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: Extend<T>,
{
    #[inline]
...
impl<K, V, const N: usize> From<[(K, V); N]> for FixedHashMap<K, V, FixedHasher>
where
    K: Eq + Hash,
{
...
impl<K, V, S> From<hashbrown::HashMap<K, V, S>> for FixedHashMap<K, V, S> {
    #[inline]
    fn from(value: hashbrown::HashMap<K, V, S>) -> Self {
        Self(value)
    }
}

impl<K, V, S> From<FixedHashMap<K, V, S>> for hashbrown::HashMap<K, V, S> {
    #[inline]
    fn from(value: FixedHashMap<K, V, S>) -> Self {
        value.0
    }
}

impl<K, V, S> Deref for FixedHashMap<K, V, S> {
...
impl<K, V, S> DerefMut for FixedHashMap<K, V, S> {
    #[inline]
...
impl<K, V, S> serde::Serialize for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: serde::Serialize,
{
    #[inline]
...
impl<'de, K, V, S> serde::Deserialize<'de> for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: serde::Deserialize<'de>,
{
    #[inline]
...
impl<K, V, S, T> FromParallelIterator<T> for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: FromParallelIterator<T>,
    T: Send,
{
    ...
    {
        Self(<hashbrown::HashMap<K, V, S> as FromParallelIterator<T>>::from_par_iter(par_iter))
    }
}

impl<K, V, S> IntoParallelIterator for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: IntoParallelIterator,
{
    type Item = <hashbrown::HashMap<K, V, S> as IntoParallelIterator>::Item;
    type Iter = <hashbrown::HashMap<K, V, S> as IntoParallelIterator>::Iter;
...
impl<'a, K: Sync, V: Sync, S> IntoParallelIterator for &'a FixedHashMap<K, V, S>
where
    &'a hashbrown::HashMap<K, V, S>: IntoParallelIterator,
{
    type Item = <&'a hashbrown::HashMap<K, V, S> as IntoParallelIterator>::Item;
    type Iter = <&'a hashbrown::HashMap<K, V, S> as IntoParallelIterator>::Iter;
...
impl<'a, K: Sync, V: Sync, S> IntoParallelIterator for &'a mut FixedHashMap<K, V, S>
where
    &'a mut hashbrown::HashMap<K, V, S>: IntoParallelIterator,
{
    type Item = <&'a mut hashbrown::HashMap<K, V, S> as IntoParallelIterator>::Item;
    type Iter = <&'a mut hashbrown::HashMap<K, V, S> as IntoParallelIterator>::Iter;
...
impl<K, V, S, T> ParallelExtend<T> for FixedHashMap<K, V, S>
where
    hashbrown::HashMap<K, V, S>: ParallelExtend<T>,
    T: Send,
{
    ...
    {
        <hashbrown::HashMap<K, V, S> as ParallelExtend<T>>::par_extend(&mut self.0, par_iter);
    }
}
...
impl<K, V, S> FixedHashMap<K, V, S> {
    ...
    pub const fn with_hasher(hash_builder: S) -> Self {
        Self(hashbrown::HashMap::with_hasher(hash_builder))
    }
    ...
    /// ```
    #[inline]
    pub fn into_inner(self) -> hashbrown::HashMap<K, V, S> {
        self.0
    }
}

impl<K, V> FixedHashMap<K, V, FixedHasher> {
