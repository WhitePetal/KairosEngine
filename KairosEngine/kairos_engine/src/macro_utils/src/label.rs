        ...
        while let Some(tokens) = unvisited.pop() {
            for t in tokens {
                match t {
                    ...
                    TokenTree::Group(group) => unvisited.push(group.stream()),
...
pub fn derive_label(
    ...
    if let syn::Data::Union(_) = &input.data {
        ...
        return quote_spanned! {
            input.span() => ::std::compile_error(#message);
        }
        .into();
    }
    ...
    where_clause.predicates.push(
        syn::parse2(quote! {
            Self: 'static + #FQSend + #FQSync + #FQClone + ::std::cmp::Eq + ::std::fmt::Debug + ::std::hash::Hash
        })
        .unwrap()
    );
    quote! {
