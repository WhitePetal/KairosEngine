...
fn the_three_states_are_representable() {
    block_on(async {
        ...
        infos.mark_failed(path.clone()).await;
        assert_eq!(
        ...
        infos
            .insert_processed(path.clone(), processed_info(1, 1, &[]))
            .await;
        assert_eq!(
            infos.get(&path).unwrap().status(),
...
fn the_graph_is_queryable_in_both_directions() {
    block_on(async {
        ...
        infos
            .insert_processed(a.clone(), processed_info(1, 10, &[]))
            .await;
        infos
            .insert_processed(b.clone(), processed_info(2, 20, &[("a.bin", 10)]))
            .await;
        infos
            .insert_processed(
                c.clone(),
...
fn reprocessing_an_asset_returns_its_dependents_for_requeueing() {
    block_on(async {
        ...
        let requeued = infos
            .insert_processed(a.clone(), processed_info(1, 11, &[]))
            .await;
...
fn stale_dependency_edges_are_cleared_on_reprocess() {
    block_on(async {
        ...
        infos
            .insert_processed(b.clone(), processed_info(2, 21, &[]))
            .await;
        assert!(!infos.get(&a).unwrap().dependents().contains(&b));
    });
}
