...
> **已撤回**：本决策在实现开始前被放弃，codebase 维持双树 + 双 `.meta` 的现状（ADR 0002 / 0004 /
> 0005 偏离 12，#239 / #240 的落地方案）。以下内容作为「为什么不该这么做」的判断留档。
...
本决策若落地会推翻 #239 / #240 落下的成品树布局；`.meta` 边车化本身（ADR 0002）保留。

**落地状态**：已撤回，实现未开始——`res/` 与运行期宿主维持双树双 `.meta` 的现状。留下的是这份
判断本身：将来若重提「成品与源同目录」，上面三个必须在代码里分支的点（事务回滚目标、
`should_process` 判据、成品后缀的初扫排除），以及「一个 `.meta` 为何可行」（`processed_info` 已是
顶层字段）都仍然成立。
