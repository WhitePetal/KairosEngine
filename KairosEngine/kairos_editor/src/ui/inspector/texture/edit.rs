...
//! to [`AssetServer::add_async`](kairos_engine::asset::AssetServer::add_async), so the
...
use kairos_engine::asset::io::get_meta_path;
use kairos_engine::asset::{Asset, AssetAction, AssetMeta, VisitAssetDependencies};

use kairos_engine::graphics::texture::{PixelDatas, Texture, TextureSettings};
...
///
/// Handed to [`AssetServer::add_async`](kairos_engine::asset::AssetServer::add_async),
...
pub(super) fn write_settings_meta(
    ...
    let meta = AssetMeta::<(), kairos_engine::graphics::texture::TextureProcessor>::new(
        AssetAction::Process {
            processor: kairos_engine::asset::meta::processor_name::<
                kairos_engine::graphics::texture::TextureProcessor,
            >()
            .to_string(),
    ...
    std::fs::write(
        ...
        kairos_engine::asset::AssetMetaDyn::serialize(&meta),
    )?;
    Ok(())
}
...
async fn read_settings(source_path: &Path) -> TextureSettings {
    ...
    match AssetMeta::<(), kairos_engine::graphics::texture::TextureProcessor>::deserialize(&bytes) {
...
mod test {
    ...
    fn settings() -> kairos_engine::graphics::texture::TextureSettings {
        kairos_engine::graphics::texture::TextureSettings {
            width: 0,
            height: 0,
