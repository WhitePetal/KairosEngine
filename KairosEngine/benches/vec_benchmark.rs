...
fn bench_float4_copy(c: &mut Criterion) {
    ...
    for (i, (x, y, z, w)) in ar.iter().enumerate() {
        ...
        group.bench_with_input(
            BenchmarkId::new("simd", i), 
            &[_float4_simd::new(*x, *y, *z, *w), _float4_simd::new(*w, *z, *x, *y)], 
            |b, input| b.iter(|| {
                let v = black_box(input[0]);
                black_box(v)
            }
        ...
        group.bench_with_input(
            BenchmarkId::new("kairos", i), 
            &[math::float4::new(*x, *y, *z, *w), math::float4::new(*w, *z, *x, *y)], 
            |b, input| b.iter(|| {
                let v = black_box(input[0]);
                black_box(v)
            }
...
fn bench_float4_dot(c: &mut Criterion) {
    ...
    for (i, (x, y, z, w)) in ar.iter().enumerate() {
        group.bench_with_input(
            ...
            |b, input| b.iter(|| {
                black_box(_float4::dot(&input[0], &input[1]))
            }
        ));
        group.bench_with_input(
            BenchmarkId::new("simid", i), 
            &[_float4_simd::new(*x, *y, *z, *w), _float4_simd::new(*w, *z, *y, *x)], 
            |b, input| b.iter(|| {
                black_box(_float4_simd::dot(&input[0], &input[1]))
            }
        ));
        group.bench_with_input(
            BenchmarkId::new("kairos", i), 
            &[math::float4::new(*x, *y, *z, *w), math::float4::new(*w, *z, *y, *x)], 
            |b, input| b.iter(|| {
                black_box(math::float4::dot(&input[0], &input[1]))
            }
        ));
    }
}
...
fn bench_float4_cross(c: &mut Criterion) {
    ...
    for (i, (x, y, z, w)) in ar.iter().enumerate() {
        group.bench_with_input(
            ...
            &[_float4::new(*x, *y, *z, *w), _float4::new(*w, *z, *y, *x)], 
            |b, input| b.iter(|| {
                black_box(_float4::cross(&input[0], &input[1]))
            }
        ));
        group.bench_with_input(
            BenchmarkId::new("simid", i), 
            &[_float4_simd::new(*x, *y, *z, *w), _float4_simd::new(*w, *z, *y, *x)], 
            |b, input| b.iter(|| {
                black_box(_float4_simd::cross(&input[0], &input[1]))
            }
        ));
        group.bench_with_input(
            BenchmarkId::new("kairos", i), 
            &[math::float4::new(*x, *y, *z, *w), math::float4::new(*w, *z, *y, *x)], 
            |b, input| b.iter(|| {
                black_box(math::float4::cross(&input[0], &input[1]))
            }
        ));
    }
}
