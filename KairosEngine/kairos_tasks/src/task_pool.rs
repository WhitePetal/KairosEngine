...
use crate::thread_executor::{ThreadExecutor, ThreadExecutorTicker};
...
impl TaskPoolBuilder {
    ...
    /// be named `<thread_name> (<thread_index>)`, i.e. `MyThreadPool (2)`
    pub fn thread_num(mut self, thread_name: String) -> Self {
...
impl TaskPool {
        ...
        T: Send + 'static,
    {
        ...
        // SAFETY: As above, all futures must complete in this function so we can change the lifetime
        let spawned: &'env ConcurrentQueue<FallibleTask<Result<T, Box<dyn std::any::Any + Send>>>> =
    ...
    async fn execute_global_external_scope<'scope, 'ticker, T>(
        ...
        let execute_forver = async move {
            loop {
                let tick_forver = async {
                    loop {
                ...
                // to get_results
                let _result = AssertUnwindSafe(executor.run(tick_forver))
                    .catch_unwind()
                    .await
                    .is_ok();
            }
        };
        get_results.or(execute_forver).await
    }
...
impl Drop for TaskPool {
    fn drop(&mut self) {
        ...
        for join_handle in self.threads.drain(..) {
            ...
            if !panicking {
                res.expect("Task thread paicked while executing.");
            }
        }
    }
}
